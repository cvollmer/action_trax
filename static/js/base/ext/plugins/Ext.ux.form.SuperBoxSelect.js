...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
            ...
            if(!this.store){
                ...
                for(var i = 0, len = opts.length;i < len; i++){
                    var o = opts[i], oe = Ext.get(o),
                        value = oe.getAttribute('value') || '',
                        cls = oe.getAttribute('className') || '',
                        style = oe.getAttribute('style') || '';
                    if(o.selected) {
                        transformValues.push(value);
                    }
                    d.push([value, o.text, cls, Ext.isString(style) ? style : style.cssText]);
                }
    ...
    findInStore : function(val){
        ...
        var index = this.store.find(this.valueField, val,0,true);
		console.log(index);
        if(index > -1){
